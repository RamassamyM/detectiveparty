    ...
    <div className="lb-card">
      ...
      {sorted.length === 0 ? (
        ...
        <div className="lb-row" key={p.id}>
          <div className="lb-pos">{MEDALS[i] || i + 1}</div>
          <Avatar player={p} size={28} />
          <div className="lb-name">{p.name}</div>
          <div className="lb-score">{p[scoreKey] || 0}</div>
        </div>
      ))}
    </div>
  )
        ...
        return (
          <div className="rank-row" key={p.id}>
            ...
            <div className="rank-info">
              ...
              <div className="rank-code">{role.n}</div>
            </div>
            <div className="rank-score">{p[scoreKey] || 0} {icon}</div>
          </div>
        )
      })}
    </div>
    ...
    <div className="podium-stages">
      {order.map((p, i) => (
        <div className={`podium-stage ${classes[i]}`} key={p.id}>
          ...
          <div style={{ fontFamily: 'Bangers, cursive', fontSize: 12, letterSpacing: 1, textAlign: 'center', marginBottom: 2 }}>{p.name}</div>
          <div style={{ fontFamily: 'Bangers, cursive', fontSize: 17, color: 'var(--y)', marginBottom: 4 }}>{p[scoreKey] || 0} {icon}</div>
          <div className="podium-block">{medals[i]}</div>
        </div>
      ))}
    </div>
  )
